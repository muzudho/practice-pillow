...
def main():
    ...
    draw = ImageDraw.Draw(im)
    # 正順
    color_list = test2_fileter(create_cos_wave(size))
    draw_tone_circle(draw, theta_list, color_list)
    im.save('shared/test2.png')

    im = Image.new('RGB', (450, 450), white)
    draw = ImageDraw.Draw(im)
    color_list = light_fileter(create_cos_wave(size))
    draw_tone_circle(draw, theta_list, color_list)
...
def test2_fileter(color_list):

    # ３本の波にフィルターを掛けます
    # return unnormalize_filter(add_filter(multiple_filter(normalize_filter(color_list), 1.1), 0.05))
    rate = 0.4
    return unnormalize_filter(add_filter(multiple_filter(normalize_filter(color_list), rate), 1-rate))
    # print(f"[{i}] {new_color[0]:02x} {new_color[1]:02x} {new_color[2]:02x}")
